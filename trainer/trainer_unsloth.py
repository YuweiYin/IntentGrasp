...
# Note: code modified from transformers==5.5.0
...
from transformers.integrations.fsdp import get_fsdp_ckpt_kwargs, update_fsdp_plugin_peft
...
from transformers.integrations.peft import MIN_PEFT_VERSION
from transformers.integrations.tpu import save_tpu_checkpoint, tpu_spmd_dataloader, wrap_model_xla_fsdp
...
from transformers.optimization import GreedyLR, get_scheduler
...
from trainer.optimization import get_scheduler  # Adafactor
from utils.models import ModelUtils
from run_eval_results import LMEval
...
class Trainer:
    ...
    Args:
        ...
        train_dataset (`torch.utils.data.Dataset` | `torch.utils.data.IterableDataset` | `datasets.Dataset`, *optional*):
            ...
            distributed fashion, your iterable dataset should either use an internal attribute `generator` that is a
    ...
    def __init__(
        ...
        if self.args.enable_jit_checkpoint:
            from transformers.trainer_jit_checkpoint import JITCheckpointCallback
        ...
        # ### IFT ###
        self.valid_random_seed = 42
        self.best_valid_score = 0.0
        self.valid_tokenizer = ModelUtils.initialize_tokenizer_hf(
            model_name=self.extra_kwargs["model_name"],
            cache_dir=self.extra_kwargs["cache_dir"],
            padding_side="left", truncation_side="left",
            verbose=self.extra_kwargs["verbose"], model_ckpt_dir=None)
        self.valid_terminators_gen = [
            self.valid_tokenizer.eos_token_id,
            # self.valid_tokenizer.convert_tokens_to_ids("<|eot_id|>")
            self.valid_tokenizer.convert_tokens_to_ids(self.valid_tokenizer.eos_token)
        ]
        self.valid_terminators_gen_set = set(self.valid_terminators_gen)
        self.valid_terminators_gen = list(self.valid_terminators_gen_set)
    ...
    def _build_accelerator_args(self, **kwargs) -> dict[str, Any]:
        ...
        if is_accelerate_available("1.2.0"):
            # if we don't have the correct version, we will rely on env var instead that were set in TrainingArguments
    ...
    def _inner_training_loop(
        ...
        if args.eval_on_start:
            ...
            self._maybe_log_save_evaluate(
                tr_loss, grad_norm, model, trial, epoch=0, ignore_keys_for_eval=ignore_keys_for_eval,
                start_time=start_time, learning_rate=learning_rate, do_valid=True, do_save=False,
            )
    ...
    def _evaluate_function(self, model):
        # ### IFT ###
        unsloth.FastLanguageModel.for_inference(model)
        model.eval()

        valid_task_name = "intent_grasp"

        valid_verbose = self.extra_kwargs["verbose"]
        valid_logger = self.extra_kwargs["logger"]
        valid_cache_dir = self.extra_kwargs["cache_dir"]
        valid_project_root_dir = self.extra_kwargs["project_root_dir"]
        valid_ckpt_save_dir = self.extra_kwargs["ckpt_save_dir"]
        valid_model_name = self.extra_kwargs["model_name"]
        valid_max_new_gen = self.extra_kwargs["max_new_gen"]
        valid_gen_temperature = self.extra_kwargs["gen_temperature"]
        valid_num = self.extra_kwargs["valid_num"]
        valid_bsz = max(1, int(self.extra_kwargs["valid_bsz"]))

        # Obtain random samples from the raw validation set
        random.seed(self.valid_random_seed)
        valid_data_all = self.eval_dataset.to_list()
        len_valid_all = len(valid_data_all)
        if isinstance(valid_num, int) and 0 < valid_num < len_valid_all:
            valid_data = random.sample(valid_data_all, valid_num)
        else:
            valid_data = valid_data_all
        self.valid_random_seed += 1

        valid_eval_obj = None
        valid_tokenizer = self.valid_tokenizer
        model.generation_config.pad_token_id = valid_tokenizer.pad_token_id

        all_prompts = []
        for item_dict in valid_data:
            assert isinstance(item_dict, dict) and len(item_dict) > 0, type(item_dict)
            # cur_input_ids = item_dict["input_ids"]
            cur_raw_info = item_dict["raw_info"]
            assert isinstance(cur_raw_info, dict) and "answer_intent" in cur_raw_info
            cur_answers = cur_raw_info["answer_intent"]
            assert isinstance(cur_answers, list) and len(cur_answers) > 0, type(cur_answers)

            cur_messages = item_dict["messages"]
            assert isinstance(cur_messages, list) and len(cur_messages) == 2
            cur_user_dialog = [cur_messages[0]]

            cur_prompt = valid_tokenizer.apply_chat_template(
                cur_user_dialog,
                tokenize=False,
                padding=False,
                add_generation_prompt=True,
                return_tensors=None,
                # enable_thinking=self.thinking_mode,  # The thinking mode of Qwen: default = True
            ...
            all_prompts.append({
                ...
                "raw_info": cur_raw_info,
                "prompt": cur_prompt,
                "answers": cur_answers,
            })

        # Split the input list into mini batches
        assert isinstance(valid_bsz, int) and valid_bsz >= 1
        prompt_batches = [all_prompts[_i: _i + valid_bsz] for _i in range(0, len(all_prompts), valid_bsz)]
        # num_batches = len(prompt_batches)
        valid_num_items = len(all_prompts)
        valid_done_cnt = 0
        valid_show_cnt = 50

        all_gen_results = dict()
        for cur_batch_idx, cur_prompt_batch in enumerate(prompt_batches):
            assert isinstance(cur_prompt_batch, list) and len(cur_prompt_batch) > 0, type(cur_prompt_batch)
            input_prompts = [cur_prompt_dict["prompt"] for cur_prompt_dict in cur_prompt_batch]  # Batch inputs

            # Run generation
            gen_results = ModelUtils.open_model_gen(
                inputs=input_prompts, model=model, tokenizer=valid_tokenizer, need_tokenize=True,
                max_new_tokens=valid_max_new_gen, temperature=valid_gen_temperature,
            )
            assert isinstance(gen_results, list) and len(gen_results) == len(cur_prompt_batch) > 0, gen_results

            for gen_res, cur_prompt_dict in zip(gen_results, cur_prompt_batch):
                assert isinstance(gen_res, dict) and len(gen_res) > 0, type(gen_res)
                valid_task_name = cur_prompt_dict["task_name"]

                gen_res["task_name"] = valid_task_name
                gen_res["answers"] = cur_prompt_dict["answers"]
                gen_res["raw_info"] = cur_prompt_dict["raw_info"]

                assert "output_text" in gen_res, gen_res.keys()
                gen_res["analysis"] = gen_res["output_text"]
                gen_res["pred_answer"] = gen_res["output_text"]

                if valid_task_name not in all_gen_results:
                    all_gen_results[valid_task_name] = [gen_res]
                else:
                    all_gen_results[valid_task_name].append(gen_res)

                valid_done_cnt += 1
                if valid_done_cnt % valid_show_cnt == 0:
                    valid_logger.info(f">>> >>> Validation Progress: [{valid_done_cnt} / {valid_num_items}]")

        all_valid_scores = []
        for valid_task_name, gen_results_with_answers in all_gen_results.items():
            # Score the model outputs
            if valid_eval_obj is None:
                valid_eval_obj = LMEval(
                    ...
                    project_root_dir=valid_project_root_dir,
                    # nltk_download_dir=nltk_download_dir,
                    model_name=valid_model_name,
                    # bsz=1,
                    debug=False,
                    output_dir=valid_ckpt_save_dir,
                    overwrite=True,
                    use_analysis=True,  # After fine-tuning, LM will give the final answer in the initial response
                )
            valid_score_dict = valid_eval_obj.lm_evaluate(
                task_name=valid_task_name, gen_results=gen_results_with_answers, do_save=False)

            assert isinstance(valid_score_dict, dict) and len(valid_score_dict) > 0
            cur_valid_score = float(valid_score_dict["all_score_stat"]["score_avg"])
            all_valid_scores.append(cur_valid_score)

        avg_valid_score = float(np.mean(all_valid_scores).item())
        # self.log({"valid_score": avg_valid_score})
    ...
    def _maybe_log_save_evaluate(
        ...
        if self.control.should_save or do_valid:
            # ### IFT ### Evaluation on the validation set
            if self.eval_dataset is not None:
                valid_score = self._evaluate_function(model)
                self.log({"valid_score": valid_score})  # "valid_epoch": epoch
                if valid_score >= self.best_valid_score:
                    logger.info(f">>> Update the checkpoint! current valid_score: {valid_score}; "
                                f"previous best_valid_score: {self.best_valid_score} !!")
                    self.best_valid_score = valid_score
                    self.log({"best_valid_score": self.best_valid_score})
                    if do_save:
                        self._save_checkpoint(model, trial)
                        self.control = self.callback_handler.on_save(self.args, self.state, self.control)
                else:
                    logger.info(f">>> Skip saving... current score: {valid_score}; "
                                f"previous best score: {self.best_valid_score}")
            ...
            [
                ...
                if os.path.isdir(os.path.join(resume_from_checkpoint, folder_name))
                   and (
                           ...
                           or os.path.isfile(
                       os.path.join(resume_from_checkpoint, folder_name, ADAPTER_SAFE_WEIGHTS_NAME))
                   )
            ]
    ...
    def _push_from_checkpoint(self, checkpoint_folder: str) -> None:
        ...
        # Saving the processing class is fast, and we don't know how many files it may have spawned,
        # so we resave it to be sure.
        if self.processing_class is not None:
            self.processing_class.save_pretrained(output_dir)
