...
# Note: code modified from transformers==4.57.3
...
def get_wsd_schedule(
    ...
    # ### IA ###
    if 0.0 < float(num_decay_steps) < 1.0:
        assert isinstance(num_training_steps, int) and num_training_steps > 0, \
            f"Error num_training_steps: {num_training_steps}"
        num_decay_steps = max(0, int(num_training_steps * float(num_decay_steps)))
    else:
        num_decay_steps = max(0, int(num_decay_steps))
