...
class TaskMultiWOZ22(TaskManager):
    ...
    def __init__(
        ...
        self.task_meta = {
            ...
            "paper_url": "https://aclanthology.org/2020.nlp4convai-1.13/",  # URL of the dataset paper
            "license": "MIT",  # the releasing license of the original dataset
            "intent_description": {
                "find restaurant": "search for places to wine and dine",
                "book restaurant": "book a table at a restaurant",
                "find attraction": "search for places to see for leisure",
                "find hotel": "search for a hotel to stay in",
                "book hotel": "book a hotel to stay in",
                "find taxi": "book taxis to travel between places",
                "find train": "search for trains that take you places",
                "book train": "book train tickets",
                "find bus": "search for a bus",
                "find hospital": "search for a medical facility or a doctor",
                "find police": "search for police station",
            },  # The description of each intent label
        }
        # Section 5: Additional annotations
        #   Besides the span annotations, we also add active user intents and requested slots for every user turn.
        ...
        self.intent_label2statement = {
            "book hotel": "To book hotel.",  # 5386,
            "book restaurant": "To book restaurant.",  # 5534,
            "book train": "To book a train.",  # 3251,
            "find attraction": "To find tourist attraction.",  # 10566,
            "find bus": "To find the bus.",  # 8,
            "find hospital": "To find the hospital.",  # 727,
            "find hotel": "To find the hotel.",  # 13004,
            "find police": "To find the police.",  # 482,
            "find restaurant": "To find the restaurant.",  # 13188,
            "find taxi": "To find the taxi.",  # 4715,
            "find train": "To find the train.",  # 12650,
        }  # intent labels --> intent statements

        self.intent_label2category = {
            "book hotel": ["daily life", "hotel"],  # 5386,
            "book restaurant": ["daily life", "restaurant"],  # 5534,
            "book train": ["daily life", "train"],  # 3251,
            "find attraction": ["daily life", "tourist attraction"],  # 10566,
            "find bus": ["daily life", "bus"],  # 8,
            "find hospital": ["daily life", "hospital"],  # 727,
            "find hotel": ["daily life", "hotel"],  # 13004,
            "find police": ["daily life", "police"],  # 482,
            "find restaurant": ["daily life", "restaurant"],  # 13188,
            "find taxi": ["daily life", "taxi"],  # 4715,
            "find train": ["daily life", "train"],  # 12650,
                    ...
                    for raw_item in cur_file_raw_data:
                        ...
                        for turn_idx, turn in enumerate(raw_item["turns"]):
                            ...
                            for frame in cur_frames:
                                ...
                                cur_intent_label = str(_cur_intent_labels[0]).strip()
                                assert cur_intent_label in self.task_meta["intent_description"]

                                assert cur_intent_label.startswith("find ") or cur_intent_label.startswith("book ")

                                cur_speaker = cur_speaker_raw
                                assert cur_speaker == "user"
                                if len(cur_context_raw) > 0:
                                    iu_context = f"### Dialogue:\n{cur_context_raw}\n{cur_text_raw}".strip()
                                    iu_question_raw = f"What is the intent of the {cur_speaker} in the dialogue?"
                                else:
                                    iu_context = cur_text_raw
