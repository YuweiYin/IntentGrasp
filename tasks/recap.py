...
class TaskRecap(TaskManager):
    ...
    def __init__(
        ...
        self.task_meta = {
            ...
            "paper_url": "https://aclanthology.org/2026.findings-eacl.105/",  # URL of the dataset paper
            "license": "BSD-3-Clause",  # the releasing license of the original dataset (ACL publication - CC-BY-NC-SA)
        ...
        self.intent_label2statement = {
            "perfect intent": "The user is expressing a clear intent.",  # 150
            "underspecified intent": "Not clear: The user's goal lacks sufficient detail.",  # 150
            "noisy input": "Not clear: The user's irrelevant or off-topic dialogue obscures the main objective.",  # 150
            "shifted intent": "Not clear: The user changes their goal during the conversation.",  # 150
            "multi intent": "Not clear: The user presents multiple distinct goals simultaneously or "
                            "sequentially without clear separation.",  # 150
    ...
    def raw_data_unification(
        ...
        for task_split in ["train", "valid", "test"]:
            ...
            for cur_filename in cur_filenames:
                ...
                cur_data = DataIO.load_json(str(cur_filepath), mode="r", encoding="utf-8", verbose=True)
                cur_split_raw_data += cur_data

            # Record intent labels
            for idx, raw_item in enumerate(cur_split_raw_data):
                cur_intent_label = str(raw_item["intent_category"]).strip()
        ...
        for task_split in ["train", "valid", "test"]:
            ...
            for cur_filename in cur_filenames:
                ...
                for raw_item in cur_file_raw_data:
                    cur_topic = str(raw_item["topic"]).strip()
                    cur_intent_label = str(raw_item["intent_category"]).strip()
                    cur_conversation = list(raw_item["conversation"])

                    cur_conversation = [str(_conv).strip() for _conv in cur_conversation]
                    assert cur_conversation[0].startswith("USER:")
                    ...
                    cur_speaker = "USER"  # "user"
                    iu_context = cur_text_raw
                    iu_question_raw = (f"In the USER-AGENT conversation, does the {cur_speaker} express a clear intent?"
                                       f" If not, what might cause the confusion or failure in planning?")
                    ...
                    if cur_topic == "flights":
                        cur_topic = "flight booking"
                    if cur_topic == "restaurants":
                        cur_topic = "restaurant booking"

                    cur_domain_topic = []
                    cur_domain = "smart assistant"
