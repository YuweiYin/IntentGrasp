...
class TaskIntentionQA(TaskManager):
    ...
    def __init__(
        ...
        self.task_meta = {
            ...
            "paper_url": "https://aclanthology.org/2024.findings-emnlp.123/",  # URL of the dataset paper
            "license": "MIT",  # the releasing license of the original dataset
            "intent_description": {},  # The description of each intent label
        }
        # Appendix C: Annotation Setup
        #   Again, we collect three votes for each QA pair and take the majority of them.
    ...
    def raw_data_unification(
        ...
        def _normalize_intent_label(intent_raw: str) -> List[str]:
            ...
            assert intent_raw.startswith("because") or intent_raw.startswith("as a result,")
        ...
        all_option_idx = {"A", "B", "C", "D", "E"}  # Each instance has 4 or 5 options
        for task_split in ["train", "valid", "test"]:
            ...
            for idx, raw_item in enumerate(cur_split_raw_data):
                cur_options = raw_item["options"]
                assert isinstance(cur_options, dict)
                cur_gold_idx = str(raw_item["gold_ind"])
                assert cur_gold_idx in all_option_idx and cur_gold_idx in cur_options
                cur_intent_label = str(cur_options[cur_gold_idx]).strip()
        ...
        for task_split in ["train", "valid", "test"]:
            ...
            for cur_filename in cur_filenames:
                ...
                for raw_item in cur_file_raw_data:
                    cur_options_raw = raw_item["options"]
                    assert isinstance(cur_options_raw, dict)
                    cur_gold_idx = str(raw_item["gold_ind"])
                    assert cur_gold_idx in all_option_idx and cur_gold_idx in cur_options_raw
                    cur_intent_label = str(cur_options_raw[cur_gold_idx]).strip()
                    _cur_intent_labels = _normalize_intent_label(cur_intent_label)
                    assert len(_cur_intent_labels) == 1
                    for _cur_intent_label in _cur_intent_labels:
                    ...
                    cur_item_a_name = str(raw_item["item_a_name"]).strip()
                    cur_item_b_name = str(raw_item["item_b_name"]).strip()
                    cur_context_raw = f"Item 1: {cur_item_a_name}\nItem 2: {cur_item_b_name}".strip()
                    cur_text_raw = f"A customer buys Item 1 and Item 2.".strip()

                    cur_speaker = "customer"
                    iu_context = f"{cur_context_raw}\n\n{cur_text_raw}".strip()
                    iu_question_raw = f"What is the most likely intention of the {cur_speaker} for buying both items?"
                    iu_answer_intent_raw = _cur_intent_labels

                    cur_domain_topic = []
                    cur_domain, cur_topic = "e-commerce", "purchase"
