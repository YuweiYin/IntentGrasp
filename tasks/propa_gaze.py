...
class TaskPropaGaze(TaskManager):
    ...
    def __init__(
        ...
        self.task_meta = {
            ...
            "paper_url": "https://aclanthology.org/2025.coling-main.376/",  # URL of the dataset paper
            "license": "MIT",  # the releasing license of the original dataset
            "intent_description": {},  # The description of each intent label
        }
        # Abstract:
        #   we present PropaGaze, a novel dataset that combines human-annotated data with
        #     high-quality synthetic data generated through a meticulously designed pipeline.
        # Section 3.1: PTC-Gaze: Human-Annotated Dataset
        #   To alleviate the annotation burden, we firstly let GPT-4 models to generate a synthetic annotation,
        #     and then let human annotators to just the generated parts of this synthetic annotation and
        #     further rewrite into their own annotations.
        #   We only have one annotator for each annotation task so no agreement rate is computed.
        # Section 3.2: RUWA-Gaze and Politifact-Gaze: Synthetic Datasets
        #   [--> these two synthetic datasets are not adopted by IntentGrasp]
        ...
        self.task_data = {
            ...
            "test": {
                "filenames": ["ptc.json"],  # "ruwa.json", "politifact.json"
                "data": [],
                "num_data": 0,
                "intents": dict(),
            }
    ...
    def raw_data_unification(
        ...
        def _normalize_intent_label(intent_raw: str) -> List[str]:
            ...
            while intent_raw.startswith("'") or intent_raw.startswith("\""):
                intent_raw = intent_raw[1:].strip()
            while intent_raw.endswith("'") or intent_raw.endswith("\""):
                intent_raw = intent_raw[:-1].strip()

            if not intent_raw.startswith("The news intends to"):
                return [""]
        ...
        for task_split in ["train", "valid", "test"]:
            ...
            for cur_filename in cur_filenames:
                ...
                assert isinstance(cur_data, list) and len(cur_data) > 0
                cur_split_raw_data += cur_data

            # Record intent labels
            for idx, raw_item in enumerate(cur_split_raw_data):
                cur_intents = list(raw_item["intents"])
                assert len(cur_intents) > 0
                cur_intent_labels = []
                for cur_intent_dict in cur_intents:
                    assert isinstance(cur_intent_dict, dict) and "intents" in cur_intent_dict
                    cur_intent_label = str(cur_intent_dict["intents"]).strip()
                    if "judgement_kitware" in cur_intent_dict:
                        cur_intent_judgement = str(cur_intent_dict["judgement_kitware"]).strip()
                        if "yes, the intent is correct" in cur_intent_judgement.lower():
                            cur_intent_labels.append(cur_intent_label)
                _cur_intent_labels = [_normalize_intent_label(_i)[0] for _i in cur_intent_labels]
                _cur_intent_labels = [_i.strip() for _i in _cur_intent_labels if len(_i.strip()) > 0]
        ...
        for task_split in ["train", "valid", "test"]:
            ...
            for cur_filename in cur_filenames:
                ...
                for raw_item in cur_file_raw_data:
                    ...
                    # Note: the news article can be very long
                    cur_text_raw = str(raw_item["content"]).strip()
                    ...
                    cur_domain_topic = []
                    cur_domain, cur_topic = "news", "propaganda"  # "propaganda in news", ""
